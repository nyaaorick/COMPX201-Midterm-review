...
* **定义:** `BF(节点) = height(节点.左子树) - height(节点.右子树)`
    * `height(空子树) = -1`
    * `height(叶节点) = 0`
    * `height(非空节点) = 1 + max(height(左孩子), height(右孩子))`
* **AVL树要求:** 每个节点的 BF 必须是 **-1, 0, 或 1**。
* **解读BF值:**
    * `BF = +1`: 左子树比右子树高1层 (左倾，L)。
    * `BF = 0`: 左右子树等高 (平衡)。
    * `BF = -1`: 右子树比左子树高1层 (右倾，R)。
    * `BF = +2`: 左子树比右子树高2层 (左重不平衡，L)。**需要旋转！**
    * `BF = -2`: 右子树比左子树高2层 (右重不平衡，R)。**需要旋转！**

## 二、AVL树不平衡类型与旋转决策

设 **P** 为**第一个** (从插入/删除点向上回溯找到的) **BF 为 +2 或 -2 的不平衡节点**。
设 **C** 为 P 的那个**更高**的子节点 (即导致不平衡的那个方向的子节点)。

1.  **左-左 (LL) 不平衡:**
    * `BF(P) = +2` (P节点左重，L)
    * `BF(C)` (即P的左孩子) `= +1` (或 `0`，但 `+1` 更典型地表示新插入导致LL)
    * **含义:** 不平衡发生在P的左孩子的左子树。
    * **旋转:** 对 **P** 进行一次 **右旋转 (Right Rotation)**。

2.  **右-右 (RR) 不平衡:**
    * `BF(P) = -2` (P节点右重，R)
    * `BF(C)` (即P的右孩子) `= -1` (或 `0`，但 `-1` 更典型地表示新插入导致RR)
    * **含义:** 不平衡发生在P的右孩子的右子树。
    * **旋转:** 对 **P** 进行一次 **左旋转 (Left Rotation)**。

3.  **左-右 (LR) 不平衡:**
    * `BF(P) = +2` (P节点左重，L)
    * `BF(C)` (即P的左孩子) `= -1` (C节点本身右倾/右重)
    * **含义:** 不平衡发生在P的左孩子的右子树。
    * **旋转:** **左-右双旋转 (Left-Right Rotation)**
        1.  先对 **C** (P的左孩子) 进行 **左旋转**。
        2.  然后对 **P** 进行 **右旋转**。

4.  **右-左 (RL) 不平衡:**
    * `BF(P) = -2` (P节点右重，R)
    * `BF(C)` (即P的右孩子) `= +1` (C节点本身左倾/左重)
    * **含义:** 不平衡发生在P的右孩子的左子树。
    * **旋转:** **右-左双旋转 (Right-Left Rotation)**
        1.  先对 **C** (P的右孩子) 进行 **右旋转**。
        2.  然后对 **P** 进行 **左旋转**。

**符号总结优化:**

* `BF = H(L) - H(R)` (平衡因子 = 左子树高 - 右子树高)
* **LL型:** `BF(P) = +2`, `BF(P.left) = +1`  => 对P右旋
* **RR型:** `BF(P) = -2`, `BF(P.right) = -1` => 对P左旋
* **LR型:** `BF(P) = +2`, `BF(P.left) = -1`  => 先对P.left左旋，再对P右旋
* **RL型:** `BF(P) = -2`, `BF(P.right) = +1`  => 先对P.right右旋，再对P左旋

**记忆技巧 (基于你的 L/R 符号):**
* 第一个字母表示不平衡节点P的倾斜方向 (L代表BF=+2, R代表BF=-2)。
* 第二个字母表示P的那个**较高子节点C**的倾斜方向 (L代表BF(C)=+1, R代表BF(C)=-1)。
    * LL: P左重，C也左倾 -> 单次右旋P
    * RR: P右重，C也右倾 -> 单次左旋P
    * LR: P左重，但C右倾 (方向不一致) -> 双旋 (先让C方向与P一致：对C左旋；然后解决P：对P右旋)
    * RL: P右重，但C左倾 (方向不一致) -> 双旋 (先让C方向与P一致：对C右旋；然后解决P：对P左旋)
